...
| GcmodAi | Every model is billed **one cent per request** (not token-metered), top-ups are 1:1. Sign-up grants 1 yuan for trial; two tiers offer strong value — ¥5 resets 30 yuan weekly, **¥20 resets 300 yuan daily** (about 30k requests a day). Kimi-K3, GPT-5.6-luna, and DeepSeek V4 Pro are available. | [Register](https://zc.gcmod.cn/sign-up?aff=4wem) |
...
| ze (Fufu Relay) | **Switched back from sub2api to NewAPI**, so model status and the model list are far clearer and the pricing page is back; in-site it calls itself "异常芙芙公益". The referral link is updated and registration needs email verification; sign-up credit and check-in amounts after the switch are unverified — see the service. | [Register](https://ai.kscsnkli.site/sign-up?aff=av7W) / [Pricing](https://ai.kscsnkli.site/pricing) |
