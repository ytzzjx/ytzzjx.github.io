...
| TokenForge (formerly TokenGate) | **Back to normal with a $20 daily check-in, now the main Claude Opus 5 route here, so it has moved up to second**; accounts were wiped by the operator, so **re-registration is required**. The injection found in earlier testing (English-only replies) has not been re-tested — check it on your first call. | [Register](https://tokenforge.ai.studio/sign-up?aff=iheu) |
...
| KKToken | **No models are left on the service, so it has moved to the bottom of the ranking until it recovers.** Same operator as JustWoker, and both lost their models at once. It previously offered $100 sign-up credit, $20 daily check-in, and Claude Opus 4.8 plus Opus 5, with a GitHub requirement. If a recovered service shows a Cloudflare block in the Claude Code CLI, turn on a proxy (set `HTTPS_PROXY` / `HTTP_PROXY` in the `env` block of `~/.claude/settings.json`, or use TUN mode). | [Register](https://kktoken.cc/sign-up?aff=vrZc) |
| JustWoker Public Service | **No models are left on the service, so it has moved to the bottom of the ranking until it recovers.** Same operator as KKToken. It previously required a GitHub account at least one year old and offered $90 sign-up credit and $20 daily check-in, focused on Claude. | [Register](https://api.justwoker.icu/sign-up?aff=T5tm) |
