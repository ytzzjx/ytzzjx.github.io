...
const siteConfig = {
  ...
  entries: [
    ...
    },
    {
      ...
      addedAt: "2026-08-31",
      updatedAt: "2026-09-02",
      updateNote: "免费额度只有注册那 1 块且没有签到，已改归付费区。",
      kind: "付费站 / 一分钱一次",
      pricing: "paid",
      name: "GcmodAi",
      summary: "全部模型按次计费，每次请求一分钱（0.01 元），不看 token 用量；充值 1:1，充值订阅可解锁 VIP 分组。注册送 1 块，约一百次请求，但没有签到，用完就得充值，所以归到付费区。含 Kimi-K3、GPT-5.6-luna 与 DeepSeek V4 Pro。",
      details:
        "计费方式是固定单价而不是倍率：全部模型统一每次请求一分钱，跟这次请求用了多少 token 无关，长上下文的任务会比较划算。注册送 1 块，按这个价算够跑一百次请求，试水够用，但没有签到，额度不会自己回来，所以这 1 块更像试用装而不是长期免费额度——按这个定位归在付费区。充值比例 1:1，另有充值订阅解锁的 VIP 分组。已知可用模型包括 Kimi-K3、GPT-5.6-luna 和 DeepSeek V4 Pro。",
      registration: "通过邀请链接注册即可，注册后送 1 块。",
      signupBonus: "1 块（约 100 次请求，仅此一次）",
      dailyCheckin: "无",
      models: "Kimi-K3 / GPT-5.6-luna / DeepSeek V4 Pro",
      experience: "全模型每次请求一分钱；免费部分只有注册那 1 块",
      caveat: "免费部分只有注册那 1 块，约一百次请求，且没有签到，额度不会自己回来，用完必须自己充值。全部模型按次计费，每次一分钱，不按 token 计量；充值 1:1，VIP 分组需充值订阅解锁。并发和具体模型清单以站内实际显示为准。",
      benefits: ["每次请求一分钱", "不按 token 计费", "注册送 1 块（约 100 次）", "无签到", "充值 1:1", "Kimi-K3", "GPT-5.6-luna", "DeepSeek V4 Pro"],
...
const entryTranslations = {
  ...
  GcmodAi: {
    kind: "Paid service / one cent per request",
    updateNote: "The free allowance is only the 1 yuan from sign-up and there is no check-in, so it moved to the paid tab.",
    summary:
      "Every model is billed per request at a flat 0.01 yuan regardless of token usage. Top-ups are 1:1, and a paid subscription unlocks a VIP group. Sign-up grants 1 yuan, about a hundred requests, but there is no check-in and you top up once it runs out — hence the paid tab. Kimi-K3, GPT-5.6-luna, and DeepSeek V4 Pro are available.",
    details:
      "Billing is a flat price per call rather than a rate multiplier: every model costs 0.01 yuan per request no matter how many tokens it used, which makes long-context work comparatively cheap. Sign-up grants 1 yuan, about a hundred requests at that price — enough to try it out — but there is no check-in, so credit never comes back on its own, which makes that 1 yuan a trial allowance rather than ongoing free credit. It is listed in the paid tab on that basis. Top-ups convert 1:1, and there is a VIP group unlocked by a paid subscription. Known models include Kimi-K3, GPT-5.6-luna, and DeepSeek V4 Pro.",
    registration: "Register through the referral link; sign-up grants 1 yuan.",
    signupBonus: "1 yuan (about 100 requests, one time only)",
    dailyCheckin: "None",
    models: "Kimi-K3 / GPT-5.6-luna / DeepSeek V4 Pro",
    experience: "One cent per request on every model; the free portion is only the 1 yuan from sign-up",
    caveat: "The free portion is only the 1 yuan from sign-up, about a hundred requests, and there is no check-in, so credit never comes back on its own — once it runs out you have to top up. All models are billed per request at 0.01 yuan, not metered by tokens; top-ups are 1:1 and the VIP group requires a paid subscription. Confirm concurrency limits and the full model list in the service itself.",
    benefits: ["One cent per request", "Not token-metered", "1 yuan on sign-up (about 100 calls)", "No check-in", "1:1 top-up", "Kimi-K3", "GPT-5.6-luna", "DeepSeek V4 Pro"],
  },
  Hubway: {
