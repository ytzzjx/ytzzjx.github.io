...
const siteConfig = {
  ...
  entries: [
    ...
    },
    {
      ...
      addedAt: "2026-08-31",
      updatedAt: "2026-09-02",
      updateNote: "补上订阅套餐：20 美元档每天重置 ¥300，按一分钱一次算非常划算。",
      kind: "半公益站 / 一分钱一次",
      name: "GcmodAi",
      summary: "全部模型按次计费，每次请求一分钱（0.01 元），不看 token 用量。注册送 1 块试水；订阅两档很划算——5 元每周重置 30 元额度，20 元每天重置 300 元额度，按一分钱一次算相当于每天三万次请求。含 Kimi-K3、GPT-5.6-luna 与 DeepSeek V4 Pro。",
      details:
        "计费方式是固定单价而不是倍率：全部模型统一每次请求一分钱，跟这次请求用了多少 token 无关，长上下文的任务会比较划算。注册送 1 块，按这个价算够跑一百次请求，试水足够。想长期用就走订阅，有两档：基础套餐 5 元，有效期一个月、每周重置额度、总额度 30 元；高级套餐 20 元，有效期一个月、每天重置额度、总额度 300 元，两档都升级到 VIP 分组。按一分钱一次折算，高级套餐每天重置的 300 元约等于三万次请求，这是把它留在公益区的理由。另外也支持普通充值，比例 1:1。已知可用模型包括 Kimi-K3、GPT-5.6-luna 和 DeepSeek V4 Pro。",
      registration: "通过邀请链接注册即可，注册后送 1 块。",
      signupBonus: "1 块（约 100 次请求）",
      dailyCheckin: "无签到；订阅后按套餐重置（高级每天、基础每周）",
      models: "Kimi-K3 / GPT-5.6-luna / DeepSeek V4 Pro",
      experience: "全模型每次请求一分钱；订阅档折算下来很划算",
      caveat: "没有签到，注册那 1 块用完额度不会自己回来，要么普通充值（1:1），要么订阅套餐。订阅套餐价格和额度都是人民币（5 元 / 30 元额度每周重置，20 元 / 300 元额度每天重置），有效期都是一个月。并发和具体模型清单以站内实际显示为准，套餐价格与额度也可能调整。",
      benefits: ["每次请求一分钱", "不按 token 计费", "注册送 1 块", "20 元档每天重置 300 元额度", "5 元档每周重置 30 元额度", "订阅升 VIP 分组", "充值 1:1", "Kimi-K3", "GPT-5.6-luna", "DeepSeek V4 Pro"],
    ...
    },
    {
      publishedAt: "2026-08-28 00:00",
      updatedAt: "2026-09-04",
      updateNote: "已从 sub2api 换回 NewAPI，模型状态和模型列表更直观；注册链接同步更新。",
      kind: "换回 NewAPI / 模型状态直观",
      name: "ze（芙芙中转站）",
      summary: "已经从 sub2api 换回 NewAPI，模型状态和模型列表比之前直观得多。站内自称「异常芙芙公益」，注册链接已更新，注册需邮箱验证。",
      details:
        "站点域名为 ai.kscsnkli.site，此前在本合集里记作 Kscsnkli AI，后按站点自称显示为 ze，现在站内名称是「异常芙芙公益」。之前换成 sub2api 后额度数据显示不准、模型情况也不好确认，现已换回 NewAPI，模型状态和模型列表都能直接在站内看清楚，定价页也回来了。注册链接已更新为下面这条，注册需要邮箱验证。换回来之后的注册赠送和签到数额我没有登录核实，以站内实际显示为准。",
      registration: "使用下面更新后的邀请链接注册；注册需要邮箱验证。",
      signupBonus: "换回 NewAPI 后未核实，以站内显示为准",
      dailyCheckin: "换回 NewAPI 后未核实，以站内显示为准",
      models: "站内模型列表与状态可直接查看",
      experience: "已换回 NewAPI，模型状态和模型列表比 sub2api 时期直观",
      caveat: "站点几次改名换型：Kscsnkli AI → ze → 站内现称「异常芙芙公益」，域名一直是 ai.kscsnkli.site。换回 NewAPI 之后的注册赠送与签到数额我没有核实，请以站内实际显示为准；使用前建议先看模型状态页确认可用性。",
      benefits: ["已换回 NewAPI", "模型状态直观", "模型列表可查", "站内称异常芙芙公益", "注册需邮箱验证", "赠送与签到待核实"],
...
const entryTranslations = {
  ...
  GcmodAi: {
    ...
    updateNote: "Subscription plans added: ¥20 resets 300 yuan daily — works out to 30k requests a day at one cent each, very cost-effective.",
    summary:
      "Every model is billed per request at a flat 0.01 yuan regardless of token usage. Sign-up grants 1 yuan for trial; two subscription tiers offer strong value — ¥5 resets 30 yuan weekly, ¥20 resets 300 yuan daily, which at one cent per request works out to 30k calls a day. Kimi-K3, GPT-5.6-luna, and DeepSeek V4 Pro are available.",
    details:
      "Billing is a flat price per call rather than a rate multiplier: every model costs 0.01 yuan per request no matter how many tokens it used, which makes long-context work comparatively cheap. Sign-up grants 1 yuan, about a hundred requests at that price — enough to try it out. For ongoing use, subscribe: the Basic tier at ¥5 lasts one month with weekly resets and a 30-yuan quota; the Premium tier at ¥20 lasts one month with daily resets and a 300-yuan quota, both unlocking the VIP group. The Premium tier's 300 yuan that resets daily works out to about 30k requests at one cent each — the reason it stays in the free tab. Regular top-ups are also available at 1:1. Known models include Kimi-K3, GPT-5.6-luna, and DeepSeek V4 Pro.",
    registration: "Register through the referral link; sign-up grants 1 yuan.",
    signupBonus: "1 yuan (about 100 requests)",
    dailyCheckin: "No check-in; subscriptions reset per plan (Premium daily, Basic weekly)",
    models: "Kimi-K3 / GPT-5.6-luna / DeepSeek V4 Pro",
    experience: "One cent per request on every model; subscription tiers offer strong value",
    caveat: "There is no check-in, so the 1 yuan from sign-up does not come back on its own — once it runs out you either top up (1:1) or subscribe. Subscription prices and quotas are both in yuan (¥5 / 30 yuan weekly reset, ¥20 / 300 yuan daily reset); term length is one month for both. All models are billed per request at 0.01 yuan, not metered by tokens. Confirm concurrency limits and the full model list in the service itself; subscription prices and quotas may also change.",
    benefits: ["One cent per request", "Not token-metered", "1 yuan on sign-up", "¥20 tier resets 300 yuan daily", "¥5 tier resets 30 yuan weekly", "VIP group on subscription", "1:1 top-up", "Kimi-K3", "GPT-5.6-luna", "DeepSeek V4 Pro"],
  },
  Hubway: {
  ...
  "ze（芙芙中转站）": {
    kind: "Back on NewAPI / clear model status",
    name: "ze (Fufu Relay)",
    updateNote: "Switched back from sub2api to NewAPI, making model status and the model list much clearer; the referral link is updated.",
    summary:
      "The service has switched back from sub2api to NewAPI, which makes model status and the model list far clearer than before. It calls itself \"异常芙芙公益\" in-site, the referral link has been updated, and registration requires email verification.",
    details:
      "The service runs at ai.kscsnkli.site and was previously listed here as Kscsnkli AI, then as ze, the name the site used; in-site it now goes by \"异常芙芙公益\". While it ran on sub2api the credit figures displayed incorrectly and model availability was hard to confirm; it has now switched back to NewAPI, so model status and the model list can be read directly on the site and the pricing page is back. The referral link has been updated to the one below, and registration requires email verification. I have not signed in to verify the sign-up credit or check-in amounts after the switch back — treat the in-site values as authoritative.",
    registration: "Use the updated referral link below; registration requires email verification.",
    signupBonus: "Unverified after the switch back; see the service",
    dailyCheckin: "Unverified after the switch back; see the service",
    models: "The model list and status can be read directly in-site",
    experience: "Back on NewAPI; model status and the model list are clearer than during the sub2api period",
    caveat: "The service has been renamed and re-platformed several times: Kscsnkli AI → ze → currently \"异常芙芙公益\" in-site, always at ai.kscsnkli.site. I have not verified the sign-up credit or check-in amounts since the switch back to NewAPI, so rely on what the service shows; check the model status page for availability before use.",
    benefits: ["Back on NewAPI", "Clear model status", "Model list visible", "Called 异常芙芙公益 in-site", "Email verification required", "Credit and check-in unverified"],
