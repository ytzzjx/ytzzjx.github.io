...
const siteConfig = {
  ...
  entries: [
    ...
    },
    {
      ...
      updateNote: "更正此前结论：KKToken / GoRouter / TabiToken 报 CF 拦截时开代理就能用，不必改用 Claude Desktop。",
      kind: "新站 / Claude Code 需走代理",
      name: "KKToken",
      summary: "JustWoker 站长的新站，注册送 100 刀、每日签到 20 刀，主打 Claude Opus 4.8 与 Opus 5；Claude Code CLI 报 CF 拦截时开代理即可，不用改用 Claude Desktop。",
      details:
        "与 JustWoker 公益站为同一站长。目前已知福利为注册送 100 刀、每日签到 20 刀，可用 Claude Opus 4.8 和 Opus 5。注册需要 GitHub 账号，具体门槛以注册页提示为准。在 Claude Code CLI 里可能报「Attention Required! | Cloudflare」——此前判断成 Claude Code 的请求头被 CF 针对、只能改用 Claude Desktop，这个结论是错的：卡住的是出网线路。开代理，并让 Claude Code 自己也走代理就能正常调用：推荐把 HTTPS_PROXY / HTTP_PROXY 写进 ~/.claude/settings.json 的 env，任何终端启动都生效；或者直接开代理客户端的 TUN 模式。同一处理对 TabiToken 和 GoRouter 一样有效。",
      registration: "需要 GitHub 账号，有账号限制，具体门槛以注册页提示为准。",
      signupBonus: "100 刀",
      dailyCheckin: "20 刀",
      models: "Claude Opus 4.8 / Opus 5",
      experience: "Claude Code CLI 报 CF 拦截，走代理后正常",
      caveat: "Claude Code CLI 可能报「Attention Required! | Cloudflare」，那是出网线路的问题，不是接口或 CLI 请求头被针对：开代理，并把 HTTPS_PROXY / HTTP_PROXY 写进 ~/.claude/settings.json 的 env（或改用 TUN 模式）即可，不必换 Claude Desktop。新站上线时间不长，额度、签到与模型规则可能调整；注册有 GitHub 限制，具体门槛以注册页为准。",
      benefits: ["新站", "JustWoker 同站长", "注册送 100 刀", "每日签到 20 刀", "Claude Opus 4.8", "Claude Opus 5", "Claude Code 需走代理", "GitHub 限制"],
    ...
    },
    {
      ...
      updatedAt: "2026-09-05",
      updateNote: "Claude Code CLI 报 CF 拦截时开代理即可，不用换客户端。",
      quietUpdate: true,
      kind: "当前开放",
      name: "TabiToken",
      summary: "目前已开放注册，邀请注册额度为 120 刀，每日签到 5-10 刀；注册窗口可能随时关闭，建议尽快尝试。",
      details:
        "主打 Claude Opus 4.8 和 Opus 5，速度快、连接稳定。签到入口：点击右上角个人头像，进入个人资料后签到。在 Claude Code CLI 里如果报「Attention Required! | Cloudflare」，那是出网线路被 CF 挡了，不是站点接口的问题：开代理，并把 HTTPS_PROXY / HTTP_PROXY 写进 ~/.claude/settings.json 的 env（任何终端启动都生效），或者直接开代理客户端的 TUN 模式。",
      registration: "当前开放注册，窗口可能随时关闭，建议尽快尝试。",
      signupBonus: "120 刀",
      dailyCheckin: "5-10 刀",
      models: "Claude Opus 4.8 / Opus 5",
      experience: "速度快、稳定",
      caveat: "Claude Code CLI 报「Attention Required! | Cloudflare」时开代理即可：把 HTTPS_PROXY / HTTP_PROXY 写进 ~/.claude/settings.json 的 env，或改用 TUN 模式。注册窗口可能随时关闭；已注册用户请尽快调用，否则有封号风险。",
      benefits: ["当前开放", "注册送 120 刀", "每日签到 5-10 刀", "个人资料内签到", "已注册请尽快调用", "报 CF 拦截时开代理"],
    ...
    },
    {
      ...
      updatedAt: "2026-09-05",
      updateNote: "稳定性下降，建议留一条备用线路；Claude Code CLI 报 CF 拦截时开代理即可。",
      quietUpdate: true,
      kind: "当前开放",
      name: "GoRouter",
      summary: "目前已开放注册，邀请注册额度为 70 刀，每日签到 5-10 刀；近期稳定性有所下降，注册窗口也可能随时关闭。",
      details:
        "主打 Claude Opus 4.8 和 Opus 5，速度仍然不错，但 2026 年 8 月 29 日起稳定性明显不如此前。签到入口：点击右上角个人头像，进入个人资料后签到。在 Claude Code CLI 里如果报「Attention Required! | Cloudflare」，那是出网线路被 CF 挡了，不是站点接口的问题：开代理，并把 HTTPS_PROXY / HTTP_PROXY 写进 ~/.claude/settings.json 的 env（任何终端启动都生效），或者直接开代理客户端的 TUN 模式。",
      registration: "当前开放注册，窗口可能随时关闭，建议尽快尝试。",
      signupBonus: "70 刀",
      dailyCheckin: "5-10 刀",
      models: "Claude Opus 4.8 / Opus 5",
      experience: "速度尚可，但近期稳定性下降",
      caveat: "近期稳定性下降，建议留一个备用线路，不要作为唯一依赖；注册窗口可能随时关闭；已注册用户请尽快调用，否则有封号风险。Claude Code CLI 报「Attention Required! | Cloudflare」时开代理即可：把 HTTPS_PROXY / HTTP_PROXY 写进 ~/.claude/settings.json 的 env，或改用 TUN 模式。",
      benefits: ["当前开放", "注册送 70 刀", "每日签到 5-10 刀", "个人资料内签到", "已注册请尽快调用", "近期稳定性下降", "报 CF 拦截时开代理"],
    ...
    },
    {
      publishedAt: "2026-08-26 00:00",
      updatedAt: "2026-09-05",
      updateNote: "已恢复正常，每日签到 20 刀，但账号被清空过，要重新注册。",
      kind: "已恢复 / 需重新注册",
      name: "TokenForge（tokengate）",
      summary: "站点已恢复正常，每日签到 20 刀；账号被站方清空过，所以要重新注册一遍。此前实测的注入问题（模型被锁成只用英文回答）这次没有复测，第一次调用时自己验一下。",
      details:
...
const entryTranslations = {
  ...
  KKToken: {
    kind: "New service / needs a proxy in Claude Code",
    updateNote: "Correction: a Cloudflare block on KKToken / GoRouter / TabiToken is cleared by turning on a proxy; Claude Desktop is not required.",
    summary:
      "A new service from the operator behind JustWoker. Sign-up grants $100 and daily check-ins add $20, with Claude Opus 4.8 and Opus 5 as the main models. When the Claude Code CLI hits a Cloudflare block, routing it through a proxy is enough.",
    details:
      "Run by the same operator as JustWoker Public Service. Known bonuses are $100 on sign-up and $20 per daily check-in, with Claude Opus 4.8 and Opus 5 available. A GitHub account is required; check the registration page for the exact requirement. The Claude Code CLI may return an \"Attention Required! | Cloudflare\" page. The earlier conclusion — that Cloudflare targets the CLI request headers and only Claude Desktop works — was wrong: what gets blocked is the network route. Turn on a proxy and make sure Claude Code uses it too. The recommended place is HTTPS_PROXY / HTTP_PROXY inside the env block of ~/.claude/settings.json, which applies to every terminal you start; TUN mode on the proxy client works as well. The same fix applies to TabiToken and GoRouter.",
    registration: "A GitHub account is required; the exact restriction is shown on the registration page.",
    signupBonus: "$100 credit",
    dailyCheckin: "$20",
    models: "Claude Opus 4.8 / Opus 5",
    experience: "The Claude Code CLI hits a Cloudflare block; fine through a proxy",
    caveat: "The Claude Code CLI may return an \"Attention Required! | Cloudflare\" page. That is the network route, not the endpoint and not the CLI request headers: turn on a proxy and set HTTPS_PROXY / HTTP_PROXY in the env block of ~/.claude/settings.json, or switch the proxy client to TUN mode. Claude Desktop is not required. The service launched recently, so credit, check-in, and model rules may change. Registration has a GitHub requirement — confirm it on the registration page.",
    benefits: ["New service", "Same operator as JustWoker", "$100 sign-up credit", "$20 daily check-in", "Claude Opus 4.8", "Claude Opus 5", "Needs a proxy in Claude Code", "GitHub requirement"],
  },
  GcmodAi: {
  ...
  TabiToken: {
    ...
    updateNote: "A Cloudflare block in the Claude Code CLI is cleared by turning on a proxy; there is no need to switch clients.",
    summary:
    ...
    details:
      "Focused on Claude Opus 4.8 and Opus 5, with fast and stable connections. To check in, open the profile photo in the top-right and go to Profile. If the Claude Code CLI returns an \"Attention Required! | Cloudflare\" page, Cloudflare is blocking the network route rather than the endpoint: turn on a proxy and set HTTPS_PROXY / HTTP_PROXY in the env block of ~/.claude/settings.json (which applies to every terminal you start), or switch the proxy client to TUN mode.",
    ...
    caveat: "An \"Attention Required! | Cloudflare\" page in the Claude Code CLI is cleared by turning on a proxy: set HTTPS_PROXY / HTTP_PROXY in the env block of ~/.claude/settings.json, or use TUN mode. The registration window may close at any time. Make an API request soon after registration or the account may be suspended.",
    benefits: ["Registration open", "$120 sign-up credit", "$5-10 daily check-in", "Check in from Profile", "Use soon after registration", "Proxy clears Cloudflare blocks"],
  },
  GoRouter: {
    kind: "Registration open",
    updateNote: "Reliability has declined; keep a backup route. A Cloudflare block in the Claude Code CLI is cleared by turning on a proxy.",
    summary:
    ...
    details:
      "Focused on Claude Opus 4.8 and Opus 5. Speed is still good, but reliability has been noticeably worse since August 29, 2026. To check in, open the profile photo in the top-right and go to Profile. If the Claude Code CLI returns an \"Attention Required! | Cloudflare\" page, Cloudflare is blocking the network route rather than the endpoint: turn on a proxy and set HTTPS_PROXY / HTTP_PROXY in the env block of ~/.claude/settings.json (which applies to every terminal you start), or switch the proxy client to TUN mode.",
    registration: "Registration is currently open and may close at any time; try it soon.",
    signupBonus: "$70 credit",
    dailyCheckin: "$5-10",
    models: "Claude Opus 4.8 / Opus 5",
    experience: "Decent speed, but reliability has declined recently",
    caveat: "Reliability has declined recently, so keep a backup route rather than relying on it alone. The registration window may close at any time, and you should make an API request soon after registration or the account may be suspended. An \"Attention Required! | Cloudflare\" page in the Claude Code CLI is cleared by turning on a proxy: set HTTPS_PROXY / HTTP_PROXY in the env block of ~/.claude/settings.json, or use TUN mode.",
    benefits: ["Registration open", "$70 sign-up credit", "$5-10 daily check-in", "Check in from Profile", "Use soon after registration", "Reliability declining", "Proxy clears Cloudflare blocks"],
  },
  "北执半公益站": {
  ...
  "TokenForge（tokengate）": {
    ...
    updateNote: "Back to normal with a $20 daily check-in, but accounts were wiped, so you have to register again.",
    summary:
      "The service is back to normal with a $20 daily check-in. Accounts were wiped by the operator, so you have to register again. The injection found in earlier testing — the model locked to English-only replies — has not been re-tested, so check it on your first call.",
    details:
