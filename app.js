...
const siteConfig = {
  ...
  entries: [
    ...
    },
    {
      ...
      kind: "新站 / 补贴期 · 注意邮件",
      name: "CheapCodex",
      summary: "注册送 20 刀、每日签到 20 刀，目前处于百亿补贴阶段；GPT 倍率 2.5x 偏贵。注册系统的邮件有问题，可能收不到验证邮件，记得翻垃圾箱。",
      details:
        "额度单位是刀，注册送 20、每日签到 20。站方目前在做百亿补贴，所以赠送给得大方，但这是阶段性的，补贴结束后额度政策可能收紧。GPT 倍率为 2.5x，也就是比标准计费贵，20 刀实际能用的量要按这个折算，别只看数字大。注册环节有个已知问题：邮件系统不稳，验证邮件可能收不到，先去垃圾箱找；实在收不到就换个邮箱或过一会儿重试。",
      registration: "邮箱注册。注册系统的邮件发送有问题，验证邮件可能进垃圾箱或收不到，请先翻垃圾箱。",
      signupBonus: "20 刀",
      dailyCheckin: "20 刀",
      models: "GPT（倍率 2.5x）",
      experience: "新站，处于补贴期；GPT 倍率 2.5x 偏贵",
      caveat: "GPT 倍率 2.5x，比标准计费贵，赠送额度的实际可用量要按倍率折算。目前的赠送力度来自百亿补贴阶段，属于阶段性政策，随时可能调整。注册时邮件系统有问题，可能收不到验证邮件，请查看垃圾箱。",
      benefits: ["注册送 20 刀", "每日签到 20 刀", "百亿补贴阶段", "GPT 倍率 2.5x", "邮件可能进垃圾箱"],
    ...
    },
    {
      publishedAt: "2026-08-26 00:00",
      updatedAt: "2026-08-31",
      updateNote: "实测有注入，模型被限制成只说英文；稳定性也差，已移到列表末尾。",
      kind: "不建议使用 / 注入锁英文",
      name: "TokenForge（tokengate）",
      summary: "原 TokenGate，改名并搬离 Manus 后已恢复访问，但实测有注入：模型被限制成只能用英文回答，中文提问也照样回英文；稳定性也差，已移到列表末尾。",
      details:
        "站点此前建在 Manus 上，因未支付账单整站无法访问，现已恢复并改名换域名。注册送 50、每日签到 15 与停摆前一致，Claude Opus 5 也确实能调。但实测有注入，表现得很直白：模型只能用英文回答，用中文提问也一样回英文——这说明请求在到达模型前被塞了额外的指令。这类站点还改了什么不好说，注册赠送再多，返回内容不可信就没有意义。这条只作留档和避坑参考，需要干活请另选一站；顺带一提，回答语言被锁死是最容易看出来的注入迹象，遇到别的站有类似表现也可以照这个思路怀疑。",
      registration: "使用新域名的注册页；GitHub 注册，主账号邮箱需为 Google 或 Microsoft 邮箱，账号注册时间需超过 14 天，注册后还需通过 Discord 认证。",
      signupBonus: "50 额度",
      dailyCheckin: "15 额度",
      models: "Claude Opus 5 可调，但被注入锁成只说英文",
      experience: "实测有注入，模型只肯用英文回答；稳定性差，不建议用于正经任务",
      caveat: "实测有注入：模型被限制成只能用英文回答，中文提问也回英文，说明请求在到达模型前被加了指令。既然能加这一条，其他改动也无从判断，不要用于需要结果可信的任务。稳定性同样较差。站点已改名换域名，请使用 tokenforge.ai.studio，旧的 manus.space 地址不要再用。",
      benefits: ["有注入", "回答被锁成英文", "稳定性差", "不建议正经使用", "注册送 50", "每日签到 15"],
...
const entryTranslations = {
  ...
  CheapCodex: {
    ...
    summary:
      "$20 on sign-up and $20 per daily check-in during a heavy subsidy push. GPT is billed at 2.5x, so it is on the expensive side. The registration email system is unreliable — verification mail may not arrive, so check your spam folder.",
    details:
      "Credit is denominated in dollars: $20 on sign-up and $20 per daily check-in. The operator is running a large subsidy campaign, which is why the grants are generous — but it is a temporary phase and the credit policy may tighten once it ends. GPT is billed at 2.5x the standard rate, so discount the headline numbers accordingly rather than reading $20 at face value. One known issue at sign-up: the mail system is flaky and the verification email may not arrive. Check the spam folder first; if it still does not show up, try another address or retry later.",
    ...
    experience: "New service in a subsidy phase; GPT at 2.5x is on the expensive side",
    caveat: "GPT is billed at 2.5x the standard rate, so the usable value of granted credit is lower than the number suggests. The current generosity comes from a subsidy campaign and is a temporary policy that may change at any time. At sign-up, the mail system may fail to deliver the verification email — check your spam folder.",
  ...
  "TokenForge（tokengate）": {
    ...
    updateNote: "Testing found injection that locks the model into English-only replies; reliability is poor too, so it moved to the end of the list.",
    summary:
      "Formerly TokenGate. The service is reachable again after renaming and moving off Manus, but testing found injection: the model only answers in English, even when asked in Chinese. Reliability is poor as well, so it has been moved to the end of the list.",
    details:
      "The service used to be hosted on Manus and was fully unreachable over an unpaid bill; it is now restored under a new name and domain. Sign-up credit of 50 and daily check-in of 15 match the pre-outage figures, and Claude Opus 5 genuinely responds. But testing shows injection in an unusually blatant form: the model answers only in English, and a Chinese prompt still comes back in English — meaning extra instructions are being inserted before the request reaches the model. What else such a service alters is anyone's guess, and generous credit means nothing if the responses cannot be trusted. This entry is kept for the record and as a warning — pick another service for real work. Worth noting for other services too: a reply language locked to one option is among the easiest injection symptoms to spot.",
    registration: "Use the registration page on the new domain. Register with GitHub; the primary email must be from Google or Microsoft, the account must be more than 14 days old, and Discord verification is required afterwards.",
    signupBonus: "50 credits",
    dailyCheckin: "15 credits",
    models: "Claude Opus 5 responds, but injection locks it to English",
    experience: "Testing found injection that limits replies to English; reliability is poor, so not recommended for real work",
    caveat: "Testing found injection: the model only replies in English, and Chinese prompts still return English, which means instructions are being added before the request reaches the model. If that can be inserted, there is no telling what else was changed — do not use this service where the result has to be trustworthy. Reliability is also poor. The service has renamed and changed domains — use tokenforge.ai.studio and stop using the old manus.space address.",
    benefits: ["Injection present", "Replies locked to English", "Poor reliability", "Not recommended", "50 sign-up credits", "15 daily check-in"],
