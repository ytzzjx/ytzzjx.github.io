...
const siteConfig = {
  ...
  entries: [
    ...
    },
    {
      publishedAt: "2026-08-26 00:00",
      updatedAt: "2026-08-31",
      updateNote: "站点因未支付 Manus 账单暂时无法访问，已移到列表末尾。",
      kind: "暂时失效 / 等待续费",
      name: "TokenGate",
      summary: "站点建在 Manus 上，目前因未支付账单整站无法访问，已移到列表末尾；站长续费后有可能恢复。原为注册送 50 额度、每日签到 15 额度。",
      details:
        "实测首页、注册页和模型状态页都被同一个账单提示页顶替，提示「Site unavailable due to unpaid billing」，需要站长在 Manus 里更新账单才能恢复。这属于托管平台欠费，不是站点被关停，所以先留在列表里观察。恢复后原有模型数量多、规格较高，但状态变化较快，建议每次使用前先打开模型状态页自行检查。",
      registration: "目前整站无法访问，等待站长续费恢复。原注册方式：GitHub 注册，主账号邮箱需为 Google 或 Microsoft 邮箱，账号注册时间需超过 14 天，注册后还需通过 Discord 认证。",
      signupBonus: "暂时无法访问；原为 50 额度",
      dailyCheckin: "暂时无法访问；原为 15 额度",
      models: "原有 DeepSeek V4 Pro / Claude Opus 5 / 其他高级模型",
      experience: "整站因未付账单不可用；恢复时间未知",
      caveat: "站点托管在 Manus，因未支付账单整站无法访问，页面提示需站长更新账单才能恢复，恢复时间未知。恢复后模型状态仍可能变化较快，使用前请先查模型状态页。",
      benefits: ["暂时无法访问", "等待站长续费", "原注册送 50", "原每日签到 15", "原有 DeepSeek V4 Pro", "原有 Claude Opus 5", "需 Discord 认证"],
...
const entryTranslations = {
  ...
  TokenGate: {
    kind: "Temporarily down / awaiting payment",
    updateNote: "The site is unreachable because of an unpaid Manus bill; moved to the end of the list.",
    summary:
      "The service is hosted on Manus and is currently unreachable because of an unpaid bill, so it has been moved to the end of the list. It may come back once the operator pays. It previously granted 50 credits on sign-up and 15 per daily check-in.",
    details:
      "In testing, the homepage, registration page, and model-status page all return the same billing notice: \"Site unavailable due to unpaid billing.\" The operator has to update billing in Manus to restore it. This is a hosting-platform payment lapse rather than a shutdown, so the entry stays on the list for now. If it returns, the service lists many advanced models but status changes quickly, so check the model-status page before each use.",
    registration: "The whole site is currently unreachable; waiting for the operator to pay. Previously: register with GitHub, with a Google or Microsoft primary email, an account older than 14 days, and Discord verification afterwards.",
    signupBonus: "Currently unreachable; previously 50 credits",
    dailyCheckin: "Currently unreachable; previously 15 credits",
    models: "Previously DeepSeek V4 Pro / Claude Opus 5 / other advanced models",
    experience: "Entire site down over an unpaid bill; restoration time unknown",
    caveat: "The service is hosted on Manus and is unreachable because of an unpaid bill; the page says the operator must update billing to restore it, and the timing is unknown. If it returns, model status may still change quickly — check the status page before use.",
    benefits: ["Currently unreachable", "Awaiting operator payment", "Previously 50 sign-up credits", "Previously 15 daily check-in", "Previously DeepSeek V4 Pro", "Previously Claude Opus 5", "Discord verification required"],
