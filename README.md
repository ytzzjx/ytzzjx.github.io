...
> 阅读英文版：[English](./README_EN.md)

好用的站点、注册方式和最新福利，一页看完，点击直达。

**使用提醒：不建议拿中转站的模型做逆向、蒸馏、涩情等操作，除非站点明确公告不禁止。** 这类用法会把站方买来的上游账号搞封，最后是整站的人一起没得用。

## 语言

页面内置中文 / English 切换，右上角有"中文 / EN"按钮，点击即可在两种语言之间切换，无需刷新。切换结果会记录在浏览器本地，下次打开仍保持上次选择。

## 现在开放注册的站点

以下是我当前整理出的仍可注册站点，最近更新过的排在前面，SeekAI 垫底。完整列表和最新状态以页面展示为准。
...
| ze（芙芙中转站） | **此前有人批量注册，现在需要通过新邀请链接重新注册**；每天有免费额度（每日额度有限制，具体上限以站内显示为准）。**日常额度主要靠去论坛 [bbs.kscsnkli.site](https://bbs.kscsnkli.site/) 做任务赚**，也有签到。模型有 GLM-5.3-Flash、GPT-5.6-sol、DeepSeek V4 Pro、GLM-5.2 与 Kimi-K3。请勿批量注册。 | [注册](https://ai.kscsnkli.site/sign-up?aff=nHwf) / [论坛](https://bbs.kscsnkli.site/) |
| sunapi 公益站 | 仅限 QQ 邮箱注册，需走邀请链接；加 QQ 群 744474016 再送 10 余额，可每天签到，每天有免费额度。绝大部分模型 0.0001 一次、分组倍率 0.05，**约等于免费**。含 GPT-5.6-sol、Qwen3.8-Flash、GLM-5.3-Flash、Muse-1.3 与官 key 的 DeepSeek-V4.1；全站约 60 个模型，有生图、生视频与嵌入模型但质量一般，具体看站内模型广场。**已更换域名**，之前的 newapi.chinahk.qzz.io 不再使用。 | [注册](https://sunapi.5201201314.top/sign-up?aff=rF1K) / [模型广场](https://sunapi.5201201314.top/pricing) |
| Orevx Engine 半公益站 | 半公益站，邮箱或手机注册送 100 算力，每日随机签到 100-200 算力；常见国产模型和 GPT、Claude 都有，调用速度快。站内同时卖支付宝/微信订阅，免费额度主要靠签到维持。可用模型很多不逐项列举，调用界面在 [llm-dashboard](https://orevx.ai/llm-dashboard/)。 | [注册](https://orevx.ai/login?mode=register&inviteCode=FAGCRGAT) |
| Ksir的小饭锅 | 公益酒馆站，邀请注册送 0.2 刀、每日签到 5 刀；站内有 Claude 和 DeepSeek 可用，可以写代码，但不可破限。 | [注册](https://api.biliksir.ggff.net/sign-up?aff=q3ep) |
...
| SeekAI | 谨慎使用，**支持 GitHub / Telegram 注册**，GitHub 新号注册送 200 刀、每日签到 20 刀，仅有 DeepSeek，疑似网页反代、工具调用异常。 | [注册](https://seekai.cc/sign-up?aff=NzMk) |

## 需要付费的站点

以下站点都需要充值才能持续使用，注册赠送只够试用。页面上切到「付费」标签就是这一批。

> **充值前先看这条**：最近 GPT 风控比较严，降智很影响体验。建议优先选有降智检测或者有说明的站点；实在不行，充值前先让模型画一个鹈鹕骑自行车的 SVG 自测一下，别钱白花了还没推进进度。
...
| AbinAPI | **推广期**：注册后加入官方 QQ 群 547911817，群内发送「/试用 你的用户名」**送 15 刀**，秒到账；**每天可在站内签到 1 次，随机到账 0.10-0.50 元**（站点程序已换成 sub2api，域名不变）。支持 GPT，**充值比例已改为 1:1**，0.5 分组约 0.05，**0.75 分组约 0.075 且相对更稳**。 | [注册](https://www.abinapi.com/sign-up?aff=9yXf) |
...
| AIHub | 本人实测，聚合多家上游可自选渠道，**渠道异常时自动切换**，适合持续工作；进群送 10 刀，L 站好评再送 10 刀，最低倍率 0.06。**进群还能看到各个渠道的智力状态**，挑渠道前可以先看一眼，避开正在降智或掺水的渠道。 | [站点](https://aihub.top/) |
...
已从页面下架的站点只留档，不再推荐访问。完整记录见 [失效站点/](./失效站点/)（由脚本从 `app.js` 的 `siteConfig.archivedEntries` 生成）。

当前归档（均不再提供访问链接）：

- Fate New API（2026-09-09 下架）：站点暂时无法使用，恢复后需重新核实。
- Denxio（2026-09-09 下架）：上游账号池全部被封，公益服务已停止。
- BaaaAI 公益站（2026-09-09 下架）：模型已连续多日无法调用，等待恢复。
- KKToken（2026-09-09 下架）：站内已无可用模型，等待恢复。
...
站点是纯静态页面，不需要构建就能部署。`scripts/` 下是四个可选的辅助脚本，只依赖 Node（图标脚本需要联网，封面脚本需要 Python + Pillow）：

| 命令 | 用途 | 什么时候跑 |
| --- | --- | --- |
| `node scripts/check-consistency.mjs` | 校验 `entries` / `displayOrder` / `entryTranslations` / 两份 README / `icons.js` 图标是否齐平 | 每次增删站点后、提交前 |
| `node scripts/build-site-meta.mjs` | 重新生成 `sitemap.xml` 和 `失效站点/README.md`，并按文件哈希刷新 HTML 里的 `?v=` 缓存参数 | 改过 `app.js` / `styles.css` / `icons.js` 后，或归档了站点后 |
| `node scripts/build-icons.mjs` | 从 lucide-static 重新打包 `icons.js` | 新增或删除 `data-lucide` 图标后 |
| `python scripts/build_og_image.py` | 重新生成分享封面 `og-cover.png` | 标题、配色或站点数量变化后 |

图标是自托管的：`icons.js` 只包含站内实际用到的 15 个 lucide 图标（约 3.3 KB），不再从 CDN 加载完整图标库。新增图标时要先把名字加进 `scripts/build-icons.mjs` 的 `ICONS` 数组再重新生成，否则 `check-consistency.mjs` 会报错。
