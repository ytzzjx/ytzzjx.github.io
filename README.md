...
以下是我当前整理出的仍可注册站点，按推荐优先级排列。完整列表和最新状态以页面展示为准。
...
| TokenForge（tokengate） | **已恢复正常，每日签到 20 刀，现在是这里主要的 Claude Opus 5 线路，排序已上调到第二位**；账号被站方清空过，**需要重新注册**。此前实测的注入（模型被锁成只用英文回答）没有复测，第一次调用时用中文问一句自己验一下。 | [注册](https://tokenforge.ai.studio/sign-up?aff=iheu) |
...
| KKToken | **站内已经没有模型了，排序降到最低，等恢复再上调。** 与 JustWoker 同站长，两站一起没了模型。原本注册送 100 刀、每日签到 20 刀，主打 Claude Opus 4.8 与 Opus 5；注册有 GitHub 限制。恢复后若在 Claude Code CLI 报 CF 拦截，开代理即可（把 HTTPS_PROXY / HTTP_PROXY 写进 `~/.claude/settings.json` 的 `env`，或改用 TUN 模式）。 | [注册](https://kktoken.cc/sign-up?aff=vrZc) |
| JustWoker 公益站 | **站内已经没有模型了，排序降到最低，等恢复再上调。** 与 KKToken 同站长。原本 GitHub 账号注册满 1 年，注册送 90 刀、每日签到 20 刀，主打 Claude。 | [注册](https://api.justwoker.icu/sign-up?aff=T5tm) |
| TabiToken | **站内已经没有模型了，排序降到最低，等恢复再上调。** 原本注册送 120 刀、每日签到 5-10 刀，支持 Claude Opus 4.8 与 Opus 5；恢复后报 CF 拦截时开代理即可。 | [注册](https://tabitoken.com/sign-up?aff=AQDR) |
| GoRouter | **站内已经没有模型了，排序降到最低，等恢复再上调。** 没模型之前稳定性就已经在下降。原本注册送 70 刀、每日签到 5-10 刀，支持 Claude Opus 4.8 与 Opus 5；恢复后报 CF 拦截时开代理即可。 | [注册](https://gorouter.app/sign-up?aff=e9NL) |
