...
| GcmodAi | 半公益站，注册送 1 块，全部模型**每次请求一分钱**（不按 token 计量）。**免费额度只有这 1 块、约一百次请求，且没有签到**，用完需自行充值；充值 1:1，充值订阅解锁 VIP 分组。含 Kimi-K3、GPT-5.6-luna 与 DeepSeek V4 Pro。 | [注册](https://zc.gcmod.cn/sign-up?aff=4wem) |
...
| TokenForge（tokengate） | 原 TokenGate，改名并搬离 Manus 后已恢复访问，注册送 50、每日签到 15。**但实测有注入：模型被锁成只用英文回答，中文提问也回英文；稳定性也差，不建议用于正经任务**，仅作留档与避坑参考。 | [注册页](https://tokenforge.ai.studio/sign-up?aff=5dyr) |
...
| AbinAPI | 付费站羊毛，注册进群送 20 刀，支持 GPT；充值比例 1:10，0.5 分组约 0.05，**0.75 分组约 0.075 且相对更稳**。 | [注册](https://www.abinapi.com/sign-up?aff=9yXf) |
